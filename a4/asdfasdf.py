...
def selectactivities(array):
    ...
    #print(*array, sep=" ")
    tmp = []
    print("The following activities are selected") # match the output
    i = len(array) - 1
...
for line in inF:
    ...
    if ' ' not in line and len(tmp) > 0: # checking where the number of activities in the set starts
        ...
        if(len(tmp[0]) <= 1):# if the tmp has results
            result = selectactivities(tmp[1:]) # pass the array without the first index
        else:
            result = selectactivities(tmp[0:]) # otherwise, pass the whole array
        result_len = len(result) # store the result
        print("Number of activities selected = " + str(result_len)) # match the output
        res = str(result)[1:-1] # list slicing to get rid of the brackets
        print("Activities: " + res + '\n')
        num_set += 1 # increase the number of the set
        tmp = [] # reset temp
    else: # case where the line is being checked has other numbers (the actual data points)
